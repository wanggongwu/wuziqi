...
public class AI implements Gobang {
            ...
            for (int c = 0; c < ARRAY[r].length; c++) {
                if (ARRAY[r][c] == 0) {// 如果该位置没有棋子则开始统计
                    ...
                    if ((countHL(r, c) + countHR(r, c) == "01100110")
                        ...
                        WEIGHTAEEAY[r][c] = WEIGHTAEEAY[r][c] + 5000;
                    }
                    ...
                    if ((countHL(r, c) + countHR(r, c) == "0100110") || (countHL(r, c) + countHR(r, c) == "0-100-1-10")
                        ...
                        WEIGHTAEEAY[r][c] = WEIGHTAEEAY[r][c] + 3000;
                    }
                    ...
                    if ((countHL(r, c) + countHR(r, c) == "0-1-1-100-10")
                        ...
                        WEIGHTAEEAY[r][c] = WEIGHTAEEAY[r][c] + 4000;
                    }
    ...
    public static String countHL(int r, int c) {
        ...
        for (int r1 = r - 1; r1 >= 0; r1--) {
            if (ARRAY[r1][c] == 0) {// 表示空位沒有棋子
                if (r1 + 1 == r) {// 相邻
                    break;
                } else {
                    code = ARRAY[r1][c] + code;// 记录棋子相连情况
                    break;
                }
            } else {// 表示该位置有棋子
                if (chess == 0) {// 第一次出现棋子
                    chess = ARRAY[r1][c];// 记录棋子
                    code = ARRAY[r1][c] + code;// 记录棋子相连情况
    ...
    public static String countHT(int r, int c) {
        ...
        for (int c1 = c - 1; c1 >= 0; c1--) {
            if (ARRAY[r][c1] == 0) {// 表示空位沒有棋子
                if (c1 + 1 == r) {// 相邻
                    break;
                } else {
                    code = ARRAY[r][c1] + code;// 记录棋子相连情况
                    break;
                }
            } else {// 表示该位置有棋子
                if (chess == 0) {// 第一次出现棋子
                    chess = ARRAY[r][c1];// 记录棋子
                    code = ARRAY[r][c1] + code;// 记录棋子相连情况
    ...
    public static String countHZXS(int r, int c) {
        ...
        for (int r1 = r - 1, c1 = c - 1; r1 >= 0 && c1 >= 0; r1--, c1--) {
            if (ARRAY[r1][c1] == 0) {// 表示空位沒有棋子
                if (c1 + 1 == c && r1 + 1 == r) {// 相邻
                    break;
                } else {
                    code = ARRAY[r1][c1] + code;// 记录棋子相连情况
                    break;
                }
            } else {// 表示该位置有棋子
                if (chess == 0) {// 第一次出现棋子
                    chess = ARRAY[r1][c1];// 记录棋子
                    code = ARRAY[r1][c1] + code;// 记录棋子相连情况
